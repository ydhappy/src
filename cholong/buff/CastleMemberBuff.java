...
public class CastleMemberBuff extends Magic{
	
	
	private int Member_AC = GameSetting.CASTLE_BUFF_AC;
	private int Member_HIT = GameSetting.CASTLE_BUFF_HIT;
	private int Member_DMG = GameSetting.CASTLE_BUFF_DMG;
	private int Member_SP = GameSetting.CASTLE_BUFF_SP;
	private int Member_REDUC = GameSetting.CASTLE_BUFF_REDUC;
	private double Member_EXP = GameSetting.CASTLE_BUFF_EXP * 0.01;
	
	
	public CastleMemberBuff(Skill skill){
		super(null, skill);
	}

	public static synchronized BuffInterface clone(BuffInterface bi, Skill skill, int time) {
		if (bi == null)
			bi = new CastleMemberBuff(skill);
		bi.setSkill(skill);
		bi.setTime(time);
		return bi;
	}
	...
	public void toBuffStart(object o){
	  if(o instanceof Character){
			...
			cha.setDynamicExp(cha.getDynamicExp() + Member_EXP);
	...
	public static void onBuff(object o, Skill skill) {
		BuffController.append(o, clone(BuffController.getPool(CastleMemberBuff.class), skill, skill.getBuffDuration()));
	}
	public static void init(Character cha, int time) {
		BuffController.append(cha, clone(BuffController.getPool(CastleMemberBuff.class), SkillDatabase.find(410), time));
	}
}