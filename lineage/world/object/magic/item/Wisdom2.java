...
public class Wisdom2 extends Magic {

	public Wisdom2(Skill skill){
		super(null, skill);
	}
	
	static synchronized public BuffInterface clone(BuffInterface bi, Skill skill, int time){
		if(bi == null)
			bi = new Wisdom2(skill);
		bi.setSkill(skill);
		bi.setTime(time);
		return bi;
	}
	...
	public void toBuffUpdate(object o) {
		if(o instanceof Character){
			...
			if(Lineage.server_version > 250)
				cha.toSender(S_BuffWisdom.clone(BasePacketPooling.getPool(S_BuffWisdom.class)));
			else
	...
	//	BuffController.remove(cha, Wafer.class);
		...
		// 지혜 적용
		BuffController.append(cha, Wisdom2.clone(BuffController.getPool(Wisdom2.class), SkillDatabase.find(502), time));
	}
}
