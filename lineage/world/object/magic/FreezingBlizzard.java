...
public class FreezingBlizzard extends IceLance {
	
	public FreezingBlizzard(Skill skill){
		super(skill);
	}
	
	static synchronized public BuffInterface clone(BuffInterface bi, Skill skill, int time){
		if(bi == null)
			bi = new FreezingBlizzard(skill);
		bi.setSkill(skill);
		bi.setTime(time);
		return bi;
	}
	...
	static public void init(MonsterInstance mi, MonsterSkill ms, int action, int effect){
		toBuff(mi, ms.getSkill(), action, ms.getRange(), effect, (int)Util.random(ms.getMindmg()* Lineage.freezing, ms.getMaxdmg() * Lineage.freezing));
	}
			...
			for(object o : cha.getInsideList(true)){
				...
				if(dmg > 0){
					...
					if(SkillController.isFigure(cha, o, skill, true, false))
						BuffController.append(o, FreezingBlizzard.clone(BuffController.getPool(FreezingBlizzard.class), skill, skill.getBuffDuration()));
				}
			}
			// 패킷 처리.
