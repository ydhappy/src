...
public class EnergyBolt {
	...
	static public void init(Character cha, object o, MonsterSkill skill, int action, int effect){
		if (o != null && SkillController.isMagic(cha, skill, true) && Util.isDistance(cha, o, skill.getRange() > 0 ? skill.getRange() : skill.getSkill().getRange())){
			//if(o instanceof PcInstance && cha instanceof PetInstance){
				toBuff(cha, o, skill.getSkill(), action, effect, (int)Util.random(skill.getMindmg(), skill.getMaxdmg() ));
			//}else{
			//	toBuff(cha, o, skill.getSkill(), action, effect, (int)Util.random(skill.getMindmg()* Lineage.eng_bolt, skill.getMaxdmg() * Lineage.eng_bolt));
		//	}
		}
	}
	
	static public void init(Character cha, object o , Skill skill,  int action, int effect , int dmg) {
		toBuff(cha, o, skill, action, effect, dmg);
	}
