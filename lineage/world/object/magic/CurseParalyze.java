...
public class CurseParalyze extends Magic {
	...
	static public void init(Character cha, int time) {
		...
		BuffController.append(cha,
				CurseParalyze.clone(BuffController.getPool(CurseParalyze.class), SkillDatabase.find(5, 0), time));
	}
	...
	static public void init(MonsterInstance mi, object o, MonsterSkill ms, int action, int effect, boolean check) {
		...
		if (SkillController.isFigure(mi, o, ms.getSkill(), true, false) && !o.isLock()) {
			...
			// 처리.
			BuffController.append(o, CurseParalyze.clone(BuffController.getPool(CurseParalyze.class), ms.getSkill(),
					ms.getBuffDuration() > 0 ? ms.getBuffDuration() : ms.getSkill().getBuffDuration() + 10));
		}
	}
	...
	static public void onBuff(Character cha, object o, Skill skill, int counter) {
		...
		if (SkillController.isFigure(cha, o, skill, true, false) && !o.isLock()) {
			...
			BuffController.append(o, CurseParalyze.clone(BuffController.getPool(CurseParalyze.class), skill,
					skill.getBuffDuration() + 10));
			return;
		}
		// \f1마법이 실패했습니다.
