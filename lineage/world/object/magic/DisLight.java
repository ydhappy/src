...
public class DisLight {
	...
	static public void init(Character cha, Skill skill, long object_id, int x, int y){
		...
		if(o!=null && Util.isDistance(cha, o, 10) && SkillController.isMagic(cha, skill, true)){
			...
			if(cha.getSix()==13){
				toBuff(cha, skill, o, 18, skill.getRange(), skill.getCastGfx(), 10,null, x, y);
			}else{
				toBuff(cha, skill, o, 18, skill.getRange(), skill.getCastGfx(), Util.random(skill.getMindmg(), skill.getMaxdmg()),null, x, y);
			}
		}
	}
	...
	static public void init(MonsterInstance mi, MonsterSkill ms, object o, int action, int effect){
		if(SkillController.isMagic(mi, ms, true) && ms.getSkill()!=null)
			toBuff(mi, ms.getSkill(), o, action, ms.getRange(), effect, (int)Util.random(ms.getMindmg()* Lineage.disslight, ms.getMaxdmg() * Lineage.disslight), null, o.getX(), o.getY());
	}
