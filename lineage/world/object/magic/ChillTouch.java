...
public class ChillTouch {
	...
	static public void init(Character cha, Skill skill, long object_id){
		...
		if(SkillController.isMagic(cha, skill, true) && SkillController.isFigure(cha, o, skill, false, false)){
			if(cha.getSix()==23){
				toBuff(cha, skill, o, 18, skill.getCastGfx(), 30);
			}else{
				toBuff(cha, skill, o, 18, skill.getCastGfx(), 0);
			}
		}
	}
	...
	static public void init(MonsterInstance mi, MonsterSkill ms, object o, int action, int effect){
		if(SkillController.isMagic(mi, ms, true) && SkillController.isFigure(mi, o, ms, false, false))
			toBuff(mi, ms.getSkill(), o, action, effect, (int)Util.random(ms.getMindmg() * Lineage.chill, ms.getMaxdmg() * Lineage.chill));
	}
	...
	static public void toBuff(Character cha, Skill skill, object o, int action, int effect, int alpha_dmg){
		...
		// 패킷 처리
		if(action > 0)
			cha.toSender(S_ObjectAttack.clone(BasePacketPooling.getPool(S_ObjectAttack.class), cha, o, action, dmg, effect, false, false, o.getX(), o.getY(), true), cha instanceof PcInstance);
	}
}
