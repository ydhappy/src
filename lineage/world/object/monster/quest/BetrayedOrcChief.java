...
public class BetrayedOrcChief extends MonsterInstance {

	private Item quest_item;
	
	static synchronized public MonsterInstance clone(MonsterInstance mi, Monster m){
		if(mi == null)
			mi = new BetrayedOrcChief();
		return MonsterInstance.clone(mi, m);
	}
	
	public BetrayedOrcChief(){
		quest_item = ItemDatabase.find("왕가의 문장 조각");
	}
	...
	protected void toAiDead(long time){
		...
		if(quest_item!=null && getAttackListSize()==1){
			...
			if(o instanceof PcInstance && !(o instanceof RobotInstance)) {
				...
				Quest q = QuestController.find(pc, Lineage.QUEST_ROYAL_LV45);  //성환 군주수정
