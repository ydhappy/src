...
public class Blob extends Slime {
	...
	public void toAttack(object o, int x, int y, boolean bow, int gfxMode, int alpha_dmg, boolean isTriple, int tripleIdx){
		super.toAttack(o, x, y, bow, gfxMode, alpha_dmg, isTriple, tripleIdx);
		...
		if (o instanceof PcInstance) {
			...
			//
			if (item != null && item.getItem().isCanbedmg() && item.getBress() != 0 && o.isBuffSoulOfFlame() == false && Util.random(0, 99) <= 10) {
				item.setDurability(item.getDurability() + 1);
				if (Lineage.server_version <= 144) {
					o.toSender(new S_InventoryEquipped( item));
					o.toSender(new S_InventoryCount( item));
				} else {
					o.toSender(new S_InventoryStatus( item));
				}
				// \f1당신의 %0%s 손상되었습니다.
				o.toSender(new S_Message( 268, item.toString()));
			}
		}
	}
	...
	public void toGiveItem(object o, ItemInstance item, long count) {
		...
		// 아이템이 강제로 들어왓을경우 인벤토리 아이템제거 시간 초기화 하기.
		//	: 안할경우 해당값이 60에 가까워져있을때 바로 삭제가 되는 문제 때문에.
		remove_item_time = 0;
	}
