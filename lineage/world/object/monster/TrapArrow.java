...
public class TrapArrow extends Character {
	...
	static synchronized public TrapArrow clone(TrapArrow ta, int[] loc, int gfx){
		...
		ta.loc = loc;
		ta.gfx = gfx;
		ta.weapon = ItemDatabase.newInstance(ItemDatabase.find(24));	// 활
		ta.arrow = ItemDatabase.newInstance(ItemDatabase.find(61));		// 화살
		ta.setStr(18);
		ta.setDex(18);
		ta.setCon(18);
		ta.setLevel(12);
	...
	protected void toAiWalk(long time) {
		...
		// 데미지 처리.
		if(target != null) {
			dmg = DamageController.getDamage(this, target, true, weapon, arrow, Util.random(10, 20), false, 0);
			DamageController.toDamage(this, target, dmg, 0);
		}
		//
		toSender(S_ObjectAttack.clone(BasePacketPooling.getPool(S_ObjectAttack.class), this, target, getGfxMode()+1, dmg, getGfx(), true, true, loc[2], loc[3], true), false);
	}
