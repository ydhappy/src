...
public class Ethin extends MonsterInstance {
	...
	public Ethin(){
		quest_item = ItemDatabase.find("붉은 열쇠");
	}
	...
	protected void toAiDead(long time){
		...
		if(quest_item!=null && getAttackListSize()==1){
			...
			if(o instanceof PcInstance && !(o instanceof RobotInstance)) {
				...
				if(pc.getClassType() == 0x01 || pc.getClassType() == 0x02){
				Quest q = QuestController.find(pc, Lineage.QUEST_TIO);
				if(q!=null && q.getQuestStep()==0 && pc.getInventory().findDbNameId(quest_item.getNameIdNumber())==null){
