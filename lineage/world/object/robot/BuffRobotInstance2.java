...
public class BuffRobotInstance2 extends RobotInstance {
	...
	// 같이 버프시전할 객체들
	protected List<BuffRobotInstance2> list;
	
	public BuffRobotInstance2() {
		//
	}
	
	public BuffRobotInstance2(int x, int y, int map, int heading, String name, int gfx) {
		...
		// 보조 케릭
		list = new ArrayList<BuffRobotInstance2>();

		for(int e=1 ; e<6 ; ++e) {
			
			BuffRobotInstance2 o = new BuffRobotInstance2();
			...
			o.setName(String.format("%s%d", "글말보조", e));
		...
		for(int e=1 ; e<7 ; ++e) {
			...
			o.setLawful(-32767);
			o.setGfx(Util.random(0, 1)==0 ? 1186 : 734);
			o.setName(String.format("%s%d", "글말버프", e));
			//o.setName(String.format(name));
			o.setTitle("2000원");
			list.add(o);
		}
		for(int e=1 ; e<6 ; ++e) {
			...
			o.setLawful(-32767);
			o.setGfx(Util.random(0, 1)==0 ? 1186 : 734);
			o.setName(String.format("%s%d", "헤이글말", e));
			//o.setName(String.format(name));
			o.setTitle("2000원");
			list.add(o);
		}
		for(int e=1 ; e<6 ; ++e) {
			...
			o.setLawful(-32767);
			o.setGfx(Util.random(0, 1)==0 ? 1186 : 734);
			o.setName(String.format("%s%d", "글말지킴이", e));
			//o.setName(String.format(name));
			o.setTitle("2000원");
			list.add(o);
		}
		...
		list_ment.add(String.format("[글말헤이샵] %d원 서비스 팍팍해드립니다.", Lineage.robot_auto_buff_aden2));
	...
	public void toDamage(Character cha, int dmg, int type, Object...opt){
		...
		ChattingController.toChatting(this, "환영합니다. 고객님", Lineage.CHATTING_MODE_NORMAL);
	...
	public void toTradeOk(Character cha){
		...
		Haste.init(list.get(20), SkillDatabase.find(6, 2), cha.getObjectId());
	}
	...
	public void toTimer(long time){
		...
		if(t!=null && t.isAden(false, Lineage.robot_auto_buff_aden2)){
			// 거래 승인 요청.
