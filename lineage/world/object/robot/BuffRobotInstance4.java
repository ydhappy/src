...
public class BuffRobotInstance4 extends RobotInstance {
	...
	// 같이 버프시전할 객체들
	protected List<BuffRobotInstance4> list;
	
	public BuffRobotInstance4() {
		//
	}
	
	public BuffRobotInstance4(int x, int y, int map, int heading, String name, int gfx) {
		...
		// 보조 케릭
		list = new ArrayList<BuffRobotInstance4>();

		for(int s=1 ; s<5 ; ++s) {
			
			BuffRobotInstance4 o = new BuffRobotInstance4();
			...
			o.setName(String.format("%s%d", "버프보조", s));
		...
		for(int s=1 ; s<5 ; ++s) {
			...
			o.setLawful(-32767);
			o.setGfx(Util.random(0, 1)==0 ? 1186 : 734);
			o.setName(String.format("%s%d", "헤이보조", s));
			//o.setName(String.format(name));
			o.setTitle("1000원");
			list.add(o);
		}
		for(int s=1 ; s<5 ; ++s) {
			...
			o.setLawful(-32767);
			o.setGfx(Util.random(0, 1)==0 ? 1186 : 734);
			o.setName(String.format("%s%d", "말섬버프", s));
			//o.setName(String.format(name));
			o.setTitle("1000원");
			list.add(o);
		}
		...
		list_ment.add(String.format("[말섬헤이샵] %d원 서비스 팍팍해드립니다.", Lineage.robot_auto_buff_aden4));
	...
	public void toDamage(Character cha, int dmg, int type, Object...opt){
		...
		ChattingController.toChatting(this, "안녕하세요. 고객님", Lineage.CHATTING_MODE_NORMAL);
	...
	public void toTimer(long time){
		...
		if(t!=null && t.isAden(false, Lineage.robot_auto_buff_aden4)){
			// 거래 승인 요청.
