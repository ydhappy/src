...
public final class SpellScrollBlessedArmor extends ItemInstance {
	
	private Skill skill;
	
	static synchronized public ItemInstance clone(ItemInstance item){
		if(item == null)
			item = new SpellScrollBlessedArmor();
		item.setSkill( SkillDatabase.find(3, 4) );
		return item;
	}
	...
	public void toClick(Character cha, ClientBasePacket cbp){
		...
		if(o!=null && o instanceof ItemArmorInstance){
			if(SkillController.isMagic(cha, skill, false, true)){
				// 처리
				BlessedArmor.onBuff(cha, (ItemInstance)o, skill, skill.getBuffDuration());
				// 수량 하향
