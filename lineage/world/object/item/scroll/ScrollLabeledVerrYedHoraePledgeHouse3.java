...
public class ScrollLabeledVerrYedHoraePledgeHouse3 extends ItemInstance {

	static synchronized public ItemInstance clone(ItemInstance item) {
		if (item == null)
			item = new ScrollLabeledVerrYedHoraePledgeHouse3();
		return item;
	}
		...
		if (LocationController.isTeleportVerrYedHoraeZone(cha, true)) {
			if (cha instanceof PcInstance) {
				...
				for (Kingdom k : KingdomController.getList()) {
					...
					if (k1 != null && k != null && k.getUid() == Lineage.KINGDOM_WINDAWOOD && k.getClanId() == k1.getClanId()) {
