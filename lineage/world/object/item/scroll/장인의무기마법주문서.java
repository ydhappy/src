...
public class 장인의무기마법주문서 extends Enchant {
	...
	public void toClick(Character cha, ClientBasePacket cbp) {
		if(cha.getInventory() != null){
			...
			if (weapon == null) {
				return;
			} else if (!weapon.getItem().getType1().equalsIgnoreCase("weapon")) {
				ChattingController.toChatting(cha, "+9 이상 무기에 사용이 가능합니다.", Lineage.CHATTING_MODE_MESSAGE);
				return;
			}
	...
	protected int toEnchant(Character cha, ItemInstance item) {
		if (item.getItem().getMaterial() != 9 && /*item.getItem().getMaterial() != 18 &&*/ item.getEnLevel() == 9)
...
//		return false;
		return Util.random(0, 100) < Lineage.jangin_weapon_9;
	}
}
