...
public class ScrollReturnTalkingIslandVillage extends ItemInstance {

	static synchronized public ItemInstance clone(ItemInstance item){
		if(item == null)
			item = new ScrollReturnTalkingIslandVillage();
		return item;
	}
	...
	public void toClick(Character cha, ClientBasePacket cbp){
		if(cha.getLevel() < 13){
			...
			if(LocationController.isTeleportVerrYedHoraeZone(cha, true)){
				LocationController.toTalkingIsland(cha);
				cha.toPotal(cha.getHomeX(), cha.getHomeY(), cha.getHomeMap());
			}
		}else{
			ChattingController.toChatting(cha, "이 아이템은 12레벨 이하일 때만 사용할 수 있습니다.", 20);
