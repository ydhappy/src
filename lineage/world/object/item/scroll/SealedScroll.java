...
public class SealedScroll extends ItemInstance {

	static synchronized public ItemInstance clone(ItemInstance item) {
		if (item == null)
			item = new SealedScroll();
		return item;
	}
	...
	public void toClick(Character cha, ClientBasePacket cbp) {
		...
		if (target instanceof ItemWeaponInstance || target instanceof ItemArmorInstance || target instanceof MagicDoll) {			
			if (target.getBress() < 0) {
				ChattingController.toChatting(cha, "봉인된 아이템에 사용할 수 없습니다.", Lineage.CHATTING_MODE_MESSAGE);
				return;
			}
			//
			...
			if (Lineage.server_version > 230) {
				...
				// %s 이(가) 봉인되었습니다.
				cha.toSender(S_Message.clone(BasePacketPooling.getPool(S_Message.class), 1004, target.toString()));
			} else {
				cha.toSender(S_InventoryEquipped.clone(BasePacketPooling.getPool(S_InventoryEquipped.class), target));
				// %s 이(가) 봉인되었습니다.
				ChattingController.toChatting(cha, Util.getStringWord(name, "이", "가") + " 봉인되었습니다.", Lineage.CHATTING_MODE_MESSAGE);
			}
			//
