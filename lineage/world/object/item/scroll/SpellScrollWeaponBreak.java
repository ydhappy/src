...
public final class SpellScrollWeaponBreak extends ItemInstance {
	
	private Skill skill;
	
	static synchronized public ItemInstance clone(ItemInstance item){
		if(item == null)
			item = new SpellScrollWeaponBreak();
		item.setSkill( SkillDatabase.find(4, 2) );
		return item;
	}
	...
	public void toClick(Character cha, ClientBasePacket cbp){
		...
		if(o != null){
			if(SkillController.isMagic(cha, skill, false, true) && Util.isDistance(cha, o, 3)){
				// 처리
				if(Util.isDistance(cha, o, 3) && SkillController.isFigure(cha, o, skill, true, false)){
