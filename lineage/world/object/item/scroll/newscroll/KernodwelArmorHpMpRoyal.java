...
public class KernodwelArmorHpMpRoyal extends ItemInstance {

	static synchronized public ItemInstance clone(ItemInstance item){
		if(item == null)
			item = new KernodwelArmorHpMpRoyal();
		return item;
	}
	...
	public void toClick(Character cha, ClientBasePacket cbp){
		...
		if(!(item.getItem().getType1().equalsIgnoreCase("armor"))){
			ChattingController.toChatting(cha, String.format("\\fT 방어구/악세에만 사용 하실 수 있습니다.", 20));
			return;
		}
		if(item.getItem().getType2().equalsIgnoreCase("roon")){
			ChattingController.toChatting(cha, String.format("\\fT 룬에 사용하실 수 없습니다.", 20));
			return;
		}
