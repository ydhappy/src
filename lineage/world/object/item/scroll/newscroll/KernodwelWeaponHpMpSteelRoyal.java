...
public class KernodwelWeaponHpMpSteelRoyal extends ItemInstance {

	static synchronized public ItemInstance clone(ItemInstance item){
		if(item == null)
			item = new KernodwelWeaponHpMpSteelRoyal();
		return item;
	}
	...
	public void toClick(Character cha, ClientBasePacket cbp){
		...
		//	if(!item.isDefinite()){
				...
				if(Lineage.server_version <= 144){
					...
					item.setAdd_Hpstell(Util.random(0, 100));
					item.setAdd_Manastell(Util.random(0, 100));
					item.setNowTime(4);
					cha.toSender(S_InventoryStatus.clone(BasePacketPooling.getPool(S_InventoryStatus.class), item));
