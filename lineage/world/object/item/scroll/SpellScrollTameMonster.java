...
public final class SpellScrollTameMonster extends ItemInstance {
	
	private Skill skill;
	
	static synchronized public ItemInstance clone(ItemInstance item){
		if(item == null)
			item = new SpellScrollTameMonster();
		item.setSkill( SkillDatabase.find(5, 3) );
		return item;
	}
	...
	public void toClick(Character cha, ClientBasePacket cbp){
		...
		if(o!=null){
			if(SkillController.isMagic(cha, skill, false, true)){
				// 처리
				TameMonster.onBuff(cha, o, skill);
				// 수량 하향
