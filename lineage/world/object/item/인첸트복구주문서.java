...
public class 인첸트복구주문서 extends ItemInstance {
						...
						if (item != null) {
							...
							if (제거할아이템 != null) {
								if (EnchantLostItemDatabase.deleteDB(el)) {
									...
									if (temp != null && (temp.getBress() != el.getBless() || temp.getEnLevel() != el.getEn_level()))
										temp = null;
									...
									ChattingController.toChatting(pc, String.format("%s 아이템이 복구되었습니다.", msg), Lineage.CHATTING_MODE_MESSAGE);
									
									if (Lineage.is_recovery_scroll && el.getScroll_name() != null) {
										Item scroll = ItemDatabase.find(el.getScroll_name());
										
										if (scroll != null) {
											ItemInstance temp_scroll = pc.getInventory().find(scroll.getName(), el.getScroll_bless(), scroll.isPiles());

											if (temp_scroll != null && temp_scroll.getBress() != el.getScroll_bless())
												temp_scroll = null;
											...
											if (temp_scroll == null) {
												...
												if (scroll.isPiles()) {
													...
													temp_scroll.setBress(el.getScroll_bless());
											...
											ChattingController.toChatting(pc, String.format("%s 아이템이 복구되었습니다.", getScrollName(el)), Lineage.CHATTING_MODE_MESSAGE);
										}
									}
...
//								ChattingController.toChatting(pc, String.format("\\f4[아이템 복구] \\f4%s(%d) \\f4이(가) 필요합니다.", 재료, 재료수량), Lineage.CHATTING_MODE_MESSAGE); // 원본
								ChattingController.toChatting(pc, String.format("소지중인 %s가 없습니다.", 재료), Lineage.CHATTING_MODE_MESSAGE);
							}
						}
					}
				} catch (Exception e) {
