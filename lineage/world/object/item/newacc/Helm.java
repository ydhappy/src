...
public class Helm extends ItemArmorInstance {
...
// 인첸을 성공했다면
		if(en != 0){
			...
			if(equipped){
				// 이전에 세팅값 빼기.
				pc.setDynamicStunDefense(pc.getDynamicStunDefense()-getDynamicStunDefense());
				// 인첸에따른 새로운값 적용.
				pc.setDynamicStunDefense(pc.getDynamicStunDefense()+new_adb);
			}
			setDynamicStunDefense(new_adb);
	...
	public void toWorldJoin(Connection con, PcInstance pc){
		...
		switch(getEnLevel()){
			...
			setDynamicStunDefense(getDynamicStunDefense()+back);
