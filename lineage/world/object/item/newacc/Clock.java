...
public class Clock extends ItemArmorInstance {
...
// 인첸을 성공했다면
		if(en != 0){
			...
			if(item.getEnchantMr() != 0)
				new_adb += getEnLevel() * item.getEnchantMr();
	...
	public void toWorldJoin(Connection con, PcInstance pc){
		...
		if(item.getEnchantMr() != 0){
			back += getEnLevel() * item.getEnchantMr();
		}

			setDynamicMr(getDynamicMr()+back);
