	...
	public void toGiveItem(object o, ItemInstance item, long count){
		...
		if (summon != null && count == 1) {
			...
			if (item instanceof EvolutionFruit) {
				if (o.getGm() == 0 && getLevel() < Lineage.evolution_level) {
					ChattingController.toChatting(o, String.format("%d레벨 이상 진화가 가능합니다.", Lineage.evolution_level), Lineage.CHATTING_MODE_MESSAGE);
					return;
				}
	...
	public void toTimer(){
		...
		if( (++food_counter%180000) == 0 ){
			...
			case Littlehungry:
				...
				ChattingController.toChatting(this, "주인님! 배가 고파요", Lineage.CHATTING_MODE_NORMAL);
				break;
			case NeitherHungryNorFull:
				food_mode = PET_FOOD_MODE.Littlehungry;
				ChattingController.toChatting(this, "슬슬 배가 고프네!", Lineage.CHATTING_MODE_NORMAL);
				break;
			case LittleFull:
				food_mode = PET_FOOD_MODE.NeitherHungryNorFull;
				break;
