...
public class InnInstance extends object {
	 ...
	 */
	private void toInn(PcInstance pc, List<InnKey> list, String action, long count, int inn_in_max, int inn_max, int inn_price){
		// 잘못된 정보라면 무시.
		...
		if(pc.getInventory().isAden(count*inn_price, true)){
			...
			pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "inn4"));
		}else{
			pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "inn3"));
		}
	}
	...
	protected boolean isOut(InnKey ik, PcInstance pc){
		...
		// 여관열쇠를 못찾았을경우도 퇴출. 20181018 여관 안튕기게
		return false;//list_temp.size()==0;
	}
