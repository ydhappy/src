...
public class SummonInstance extends MonsterInstance {
	...
	public void toAi(long time){
		...
		if(Lineage.ai_auto_healingpotion_percent >= now_percent){
			...
			if(ii!=null && ii.isClick(this))
				ii.toClick(this, null);
		}
		
		// 
		Buff buff = BuffController.find(this);
		if(buff!=null && buff.find(Haste.class) == null) {
			ItemInstance ii = getInventory().find(HastePotion.class);
			if(ii!=null && ii.isClick(this))
				ii.toClick(this, null);
		}
					...
					if(!Util.isDistance(this, summon.getMaster(), 1)){
						...
						for(ItemInstance ii : getInventory().getList()) {
							...
							if(summon.getMaster().getInventory().isAppend(ii, ii.getCount(), false)){
								...
								if(temp != null){
									summon.getMaster().getInventory().count(temp, ii, temp.getCount()+ii.getCount(), true);
								}else{
									// 전체 이동
									temp = ii;
									// 처리할 아이템 새로 등록.
						...
						// 인벤에서 전달된 아이템 제거.
						getInventory().removeList(list_temp);
						list_temp.clear();
						// 휴식모드로 전환.
						setSummonMode(SUMMON_MODE.Rest);
					}
