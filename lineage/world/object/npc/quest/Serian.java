...
public class Serian extends FirstQuest {

	public Serian(Npc npc){
		super(npc);
	...
	public void toTalk(PcInstance pc, ClientBasePacket cbp) {
		if(pc.getClassType()==0x00 || pc.getClassType()==0x03 || pc.getClassType()==0x05) {
			...
			if(q.getQuestStep() == 0)
				pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "orenb1"));
			else
				pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "orena" + toErrorHtml(0, q.getQuestStep())));
		} else {
			pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "orenb12"));
		}
	}
		...
		if(action.equalsIgnoreCase("0")){
			...
			if(q!=null && q.getQuestStep()==0){
				...
				if(CraftController.isCraft(pc, l, true)){
					...
					q.setQuestStep( 1 );
					// 창 띄우기.
					pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "orenb0"));
				}
			}
		}
	}
