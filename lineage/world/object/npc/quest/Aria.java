...
public class Aria extends QuestInstance {
	...
	public void toTalk(PcInstance pc, String action, String type, ClientBasePacket cbp, Object...opt){
		boolean isItemRemove = opt!=null&&opt.length>0 ? (boolean)opt[0] : true;
		boolean isMent = opt!=null&&opt.length>1 ? (boolean)opt[1] : true;
		...
		if(action.equalsIgnoreCase("quest 13 aria2")){
			...
			if(craft!=null && q!=null && q.getQuestStep()==1){
				...
				// 재료 확인.
				if(!isItemRemove || CraftController.isCraft(pc, l, true)){
					// 재료 제거
					if(isItemRemove)
