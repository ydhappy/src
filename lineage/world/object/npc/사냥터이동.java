...
public class 사냥터이동 extends object {
	...
	public void toTalk(PcInstance pc, ClientBasePacket cbp) {
		...
		for (DungeonBook db : DungeonBookTeleportDatabase.getList()) {
			if (db.getAden() != null && ItemDatabase.find(db.getAden()) != null && db.getCount() > 0) {
				...
				//msg.add(String.format("%s (%s)", db.getName(), db.getLevel()));
		...
		pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "dunbook", null, msg));
	}
		...
		try {
			...
			if (db != null) {
				if (pc.getLevel() < db.getLevel()) {
					ChattingController.toChatting(pc, String.format("해당 맵은 %d레벨 이상 입장 가능합니다.", db.getLevel()), Lineage.CHATTING_MODE_MESSAGE);
					return;
				}
				
				if (db.isClan()) {
					if (pc.getClanId() < 1) {
						ChattingController.toChatting(pc, "해당 맵은 혈맹 가입자만 입장 가능합니다.", Lineage.CHATTING_MODE_MESSAGE);
						return;
					}
				}
				...
				if (db.isWanted()) {
					if (!WantedController.checkWantedPc(pc)) {
						ChattingController.toChatting(pc, "해당 맵은 수배자만 입장 가능합니다.", Lineage.CHATTING_MODE_MESSAGE);
						return;
					}
				}
