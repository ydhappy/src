...
public class 장비스왑 extends object {
	...
	public void toTalk(PcInstance pc, String action, String type, ClientBasePacket cbp, Object...opt){
		if (pc != null && !pc.isWorldDelete() && !pc.isLock() && !pc.isDead() && !pc.isFishing()) {
			...
			if (action.equalsIgnoreCase("save swap")) {
				if (pc.getSwap().size() >= 5) {
					ChattingController.toChatting(pc, "\\fY장비 스왑 등록은 최대 5개까지 가능합니다.", Lineage.CHATTING_MODE_MESSAGE);
					return;
				}
				
				ChattingController.toChatting(pc, "\\fY[장비 스왑 등록] 명칭을 입력하시기 바랍니다. ex) 물방셋", Lineage.CHATTING_MODE_MESSAGE);
				Swap_Plugin.setAction(action);
...
//					swap.add(String.format("반지: %s", swapList[Lineage.SLOT_RING1] == null ? "등록된 아이템 없음" : Util.getItemNameToString(pc, swapList[Lineage.SLOT_RING1])));
//					swap.add(String.format("반지: %s", swapList[Lineage.SLOT_RING2] == null ? "등록된 아이템 없음" : Util.getItemNameToString(pc, swapList[Lineage.SLOT_RING2])));
...
//					swap.add(String.format("가더: %s", swapList[Lineage.SLOT_GARDER] == null ? "등록된 아이템 없음" : Util.getItemNameToString(pc, swapList[Lineage.SLOT_GARDER])));
//					swap.add(String.format("화살: %s", swapList[Lineage.SLOT_ARROW] == null ? "등록된 아이템 없음" : Util.getItemNameToString(pc, swapList[Lineage.SLOT_ARROW])));
				...
				ChattingController.toChatting(pc, String.format("[%s] 장비 스왑 삭제 완료.", pc.selectSwap), Lineage.CHATTING_MODE_MESSAGE);
				this.toTalk(pc, null);
	...
	public void equipItem(PcInstance pc){
		...
		this.toTalk(pc, null);
		pc.toSender(S_SoundEffect.clone(BasePacketPooling.getPool(S_SoundEffect.class), 2827)); 
		ChattingController.toChatting(pc, String.format("[%s] 장비 착용 완료.", pc.selectSwap), Lineage.CHATTING_MODE_MESSAGE);
	}
}
