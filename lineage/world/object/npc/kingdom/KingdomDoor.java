...
public class KingdomDoor extends Character {
	...
	public void toDamage(Character cha, int dmg, int type, Object...opt){
		...
		if(cha.getClanId()!=0 && kingdom.getClanId()==cha.getClanId())
			return;
		// 주변 성경비에게 도움 요청.
		for(object inside : getInsideList(true)){
			if(inside instanceof KingdomGuard && inside.getClanId()==getClanId())
				inside.toDamage(cha, 0, 3);
		}
		// 손상 처리.
		if(type==0 && cha.isBuffSoulOfFlame()==false){
			ItemInstance weapon = cha.getInventory().getSlot(8);
			if(weapon!=null && weapon.getItem().isCanbedmg() && Util.random(0, 100) < 8 && weapon.getBress() != 0){
