...
public class SpotTower extends Character {
	...
	public SpotTower() {
		...
		monsterNameList = new String[][] {{"킹 버그베어(5)", "버그베어(5)", "켈베로스(5)", "구울(5)"},					// 1단계 스폰 몬스터
										{"코카트리스(5)", "바실리스크(5)"},													// 2단계 스폰 몬스터
										{"드레이크(1)"},														// 3단계 스폰 몬스터
										{"드레이크(2)"}};																// 4단계 스폰 몬스터
	}
	...
	public void start(long time) {
		SpotController.spotClanId = 0;
		setMaxHp(Lineage_Balance.spot_tower_hp);
		setNowHp(Lineage_Balance.spot_tower_hp);
		
		isStart = true;
		endTime = time + (1000 * Lineage_Balance.spot_tower_time);
		toTeleport(Lineage_Balance.spot_tower_x, Lineage_Balance.spot_tower_y, Lineage_Balance.spot_tower_map, false);
