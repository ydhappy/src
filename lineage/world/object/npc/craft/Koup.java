...
public class Koup extends CraftInstance {
	...
	public Koup(Npc npc){
		...
		// 제작 처리 초기화.
		Item i = ItemDatabase.find("어둠의 이도류");
		if(i != null){
		...
		if(i != null){
			...
			List<Craft> l = new ArrayList<Craft>();
			l.add( new Craft(ItemDatabase.find("흑요석"), 100) );
			l.add( new Craft(ItemDatabase.find("강암석"), 10) );
			l.add( new Craft(ItemDatabase.find("철괴"), 10) );
		...
		if(i != null){
			craft_list.put("request silver dualblade", i);
		...
		if(i != null){
			craft_list.put("request platinum bar", i);
		...
		if(i != null){
			...
			l.add( new Craft(ItemDatabase.find("블랙 미스릴 판금"), 3) );
			l.add( new Craft(ItemDatabase.find("고급 에메랄드"), 30) );
	...
	public void toTalk(PcInstance pc, ClientBasePacket cbp) {
		if(pc.getClassType() == 0x04)
			...
			pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "koup2"));
	}
