...
public class FakeBarlog extends CraftInstance {

	public FakeBarlog(Npc npc){
		super(npc);
		...
		if(i != null){
			craft_list.put("request new cursed blood", i);
		...
		i = ItemDatabase.find("회백색의 그림자 신전 3층 열쇠");
		if(i != null){
			craft_list.put("request key31 tos", i);
			
			List<Craft> l = new ArrayList<Craft>();
			l.add( new Craft(ItemDatabase.find("죽음의 머리카락"), 1) );
			l.add( new Craft(ItemDatabase.find("영혼석 파편"), 1000) );
			list.put(i, l);
		}
		
		i = ItemDatabase.find("푸른색의 그림자 신전 3층 열쇠");
		if(i != null){
		...
		i = ItemDatabase.find("붉은색의 그림자 신전 3층 열쇠");
		if(i != null){
