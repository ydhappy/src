...
public class KuberaQuest extends object {
	...
	public void showHtml(PcInstance pc) {
		...
		case 1:
			...
			// 보상
			kqlist.add(String.format("%s %d", Lineage.q1 ,Lineage.qc1));
			break;
		case 2:
			// 진행번호
			...
			kqlist.add(String.format("%d", pc.getQuestKill()));
			// 보상
			kqlist.add(String.format("%s %d", Lineage.q2 ,Lineage.qc2));
			break;
		case 3:
			// 진행번호
			...
			kqlist.add(String.format("%d", pc.getQuestKill()));
			// 보상
			kqlist.add(String.format("%s %d", Lineage.q3 ,Lineage.qc3));
			break;
	...
	public void toTalk(PcInstance pc, String action, String type, ClientBasePacket cbp, Object...opt){
		int monstkill = 0;
		if (action.equalsIgnoreCase("kquest-go")) {
			Random random = new Random();
			int i = random.nextInt(3) + 1;
			
			if(pc.getQuestChapter() == 0) {
				ChattingController.toChatting(pc, String.format("\\fY [퀘스트] 수락된 퀘스트가 없어 이동이 불가능합니다"), Lineage.CHATTING_MODE_MESSAGE);
				return;
			}
