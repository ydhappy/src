...
public final class AgitController {
	...
	static public boolean isAgitLocation(object o) {
		return isAgitLocation(o.getX(), o.getY(), o.getMap());
	}

	static public boolean isAgitLocation(object o, int idx) {
		return Lineage.AGITLOCATION[idx][1] <= o.getX() && Lineage.AGITLOCATION[idx][2] >= o.getX()
				&& Lineage.AGITLOCATION[idx][3] <= o.getY() && Lineage.AGITLOCATION[idx][4] >= o.getY()
				&& 4 == o.getMap();
	}

	static public boolean isAgitLocation(int x, int y, int map) {
		for (int[] i : Lineage.AGITLOCATION) {
			if (i[1] <= x && i[2] >= x && i[3] <= y && i[4] >= y && 4 == map)
				return true;
		}
		return false;
	}
