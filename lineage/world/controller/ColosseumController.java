...
public final class ColosseumController {
	...
	static private void toGiran(){
		...
		c.setTimeStart(new int[] { 5, 17 }); // 콜롯세움이 활성화될 리니지월드 시간. 시단위
		...
		c.setStageItemCount(new int[][]{
				// 1군
				{ 50000, 50000, 50000, 50000, 50000, 50000, // 아데나
						10, 10, 10, // 은원석 조각
						50, 50, 50, 50, 50, 50, // 고빨
						30, 30, 30, 30, 30, 30, // 고주
						15, 15, 15, 15, 15, 15, // 고말
						10, 10, 10, // 숫돌
						10, 10, 10 }, // 비취
				// 2군
				{ 100000, 100000, 100000, 100000, 100000, 100000, // 아데나
						3, 3, 3, // 황금 원석 조각
						100, 100, 100, 100, 100, 100, // 고빨
						60, 60, 60, 60, 60, 60, // 고주
						30, 30, 30, 30, 30, 30, // 고말
						10, 10, 10, // 숫돌
						10, 10, 10, // 비취
						20, 20, 20, 20, 20, 20 }, // 빈주문서
		});
		// 1군 아이템
					...
					switch(c.getStatus()){
						...
						case 시작:
							if(c.getMaxStage() <= c.nowStage){
								...
								if(++c.timer_ment_cnt%60 == 0){
									...
									if(cnt > 0){
										// 5분 후에 최종전이 시작됩니다.
										World.toSender(S_ObjectChatting.clone(BasePacketPooling.getPool(S_ObjectChatting.class), null, Lineage.CHATTING_MODE_MESSAGE, String.format("콜롯세움 관리자: %d분 후에 최종전이 시작됩니다.", cnt)), c.getMap());
									}else{
										// 상태 변경.
								...
								if(c.list_spawn_idx<list_spawn_max && c.timer_ment_cnt++%spawn_sleep == 0){
									...
									for(int i=0 ; i<count ; ++i){
										...
										if(mi != null){
											...
											MonsterSpawnlistDatabase.toSpawnMonster(mi, World.get_map(c.getMap()), true, c.getX(), c.getY(), c.getMap(), Lineage.SEARCH_MONSTER_TARGET_LOCATION, 0, false, true, true, true);
										}
									}
									// 값 갱신.
									c.list_spawn_idx += 1;
								...
								if(c.timer_cool_time==0 && c.list_spawn_idx>=list_spawn_max){
									// 제 1 군의 투입이 완료되었습니다.
									World.toSender(S_ObjectChatting.clone(BasePacketPooling.getPool(S_ObjectChatting.class), null, Lineage.CHATTING_MODE_MESSAGE, String.format("콜롯세움 관리자: 제 %d 군의 투입이 완료되었습니다.", nowStage)), c.getMap());
									// 아이템 드랍.
						...
						case 최종전:
							if(c.timer_ment_cnt == 0){
								...
								for(int i=0 ; i<c.getListBoss().size() ; ++i){
									...
									for(int j=0 ; j<count ; ++j){
										...
										if(mi != null){
											c.getListSpawn().add(mi);
											MonsterSpawnlistDatabase.toSpawnMonster(mi, World.get_map(c.getMap()), true, c.getX(), c.getY(), c.getMap(), Lineage.SEARCH_MONSTER_TARGET_LOCATION, 0, true, true, true, true);
										}
									}
								}
							}
							...
							if(++c.timer_ment_cnt%60 == 0){
								...
								if(cnt > 0){
									// 경기 종료까지 4분 남았습니다.
									World.toSender(S_ObjectChatting.clone(BasePacketPooling.getPool(S_ObjectChatting.class), null, Lineage.CHATTING_MODE_MESSAGE, String.format("콜롯세움 관리자: 경기 종료까지 %d분 남았습니다.", cnt)), c.getMap());
								}else{
									// 상태 변경.
