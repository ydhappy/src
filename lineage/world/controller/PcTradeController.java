...
//	static private List<PcTrade> pool = new ArrayList<PcTrade>();
//	static private long checkTime;
//	
//	static public PcTrade getPool() {
//		PcTrade pt = null;
//		
//		synchronized (pool) {
//			if (pool.size() > 0) {
...
//	}
//	
//	static public void setPool(PcTrade pt) {
//		synchronized (pool) {
//			if(!pool.contains(pt))
//				pool.add(pt);
//		}
...
//			}
//			
//			pc.getPay_info1(st.nextToken());		
//			if (pc.getPay_info1().length() < 2) {
//				ChattingController.toChatting(o, "이름의 길이가 짧습니다.", Lineage.CHATTING_MODE_MESSAGE);
//				return;
//			} else if (!Pattern.matches("^[가-힣]*$", pc.getPay_info1())) {
//				ChattingController.toChatting(o, "이름은 한글만 가능합니다.", Lineage.CHATTING_MODE_MESSAGE);
//				return;
//			}
//		
//			pc.getPay_info1(st.nextToken());	
//			if (!Pattern.matches("^01(?:0|1|[6-9])-(?:\\d{3}|\\d{4})-\\d{4}$", pc.getPay_info1())) {
//				ChattingController.toChatting(o, "핸드폰 번호 형식이 잘못되었습니다. ex) 010-1234-5678", Lineage.CHATTING_MODE_MESSAGE);
//				return;
//			}
//			
//	.setInfoBankNum(st.nextToken());
//			if (pc.getInfoBankNum().length() < 2) {
//				ChattingController.toChatting(o, "계좌번호의 길이가 짧습니다.", Lineage.CHATTING_MODE_MESSAGE);
//				return;
//			}
//			
//			ChattingController.toChatting(o, String.format("등록완료. %s / %s / %s / %s", pc.getInfoName(), pc.getInfoPhoneNum(), pc.getInfoBankNum()), Lineage.CHATTING_MODE_MESSAGE);
//		} catch (Exception e) {
//			// TODO: handle exception
//			ChattingController.toChatting(o, Lineage.command + "계좌등록 홍길동 010-1234-5678 신한 110-365-123456", Lineage.CHATTING_MODE_MESSAGE);
//		}
//	}
//	
//	/**
...
//			ChattingController.toChatting(pc, "[" + Lineage.command + "계좌등록]에서 정보를 등록해주시기 바랍니다.", Lineage.CHATTING_MODE_MESSAGE);
//			return false;
//		}
//		return true;
//	}
...
//		} catch (Exception e) {
//			// TODO: handle exception
//			ChattingController.toChatting(o, "등록된 정보가 없습니다. " + "[" + Lineage.command + "계좌등록] 에서 정보를 등록해주시기 바랍니다.", Lineage.CHATTING_MODE_MESSAGE);
//		}
//	}
//	
//	/**
...
//			int price = Integer.valueOf(st.nextToken());
//			long itemCount = Long.valueOf(st.nextToken());
//			String subject = String.format("%,d만 아덴 팝니다.", itemCount / 10000);
//			
//			if (subject == null || subject.length() < 1) {
//				ChattingController.toChatting(o, "제목이 잘못되었습니다.", Lineage.CHATTING_MODE_MESSAGE);
//				return;
...
//		}
//		
//		if ((item.getItem().getNameIdNumber() == 4 || item.getItem().getName().equalsIgnoreCase("아데나")) && 
//			Lineage.is_aden_static_price && Lineage.aden_static_unit > 0 && Lineage.aden_static_unit >= 10000) {
...
//				
//				pc.getInventory().count(item, item.getCount() - pc.getPcTrade().getCount(), true);
//				
...
//			st.setString(14, trade.getName());
//			st.setString(15, trade.getPhone_num());
//			st.setString(16, trade.getBank_name());
//			st.setString(17, trade.getBank_num());
//			st.executeUpdate();
//			st.close();
//			
...
//
//			// 제거.
//			st = con.prepareStatement("SELECT * FROM pc_trade WHERE sell_account_uid=?");
//			st.setLong(1, pc.getAccountUid());
//			rs = st.executeQuery();
//			
...
//
//			// 제거.
//			st = con.prepareStatement("SELECT * FROM pc_trade WHERE buy_account_uid=?");
//			st.setLong(1, pc.getAccountUid());
//			rs = st.executeQuery();
//			
...
//				
//				if (buy != null) {
//					ItemInstance temp = buy.getInventory().find(i.getName(), bless, i.isPiles());
//					
//					if (temp == null) {
//						// 겹칠수 있는 아이템이 존재하지 않을경우.
//						if (i.isPiles()) {
