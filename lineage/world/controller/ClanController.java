...
public final class ClanController {
	...
	static public void toOut(PcInstance pc) {
		if (pc.getMap() == Lineage.battle_zone_map)
			return;
		Clan c = find(pc);
		if (c != null) {
			for (Kingdom k : KingdomController.getList()) {
//				if (k != null && k.isWar()) {
//					ChattingController.toChatting(pc, "공성중에는 혈맹 탈퇴를 하실 수 없습니다.", Lineage.CHATTING_MODE_MESSAGE);
//					return;
//				}
			}
				...
				if (k != null && !Lineage.kingdom_clan_join) {
					...
					if (c != null) {
						if (!Lineage.is_two_clan_join && !CharactersDatabase.isClanJoin(pc, c.getName())) {
							ChattingController.toChatting(pc, "계정 내 다른 캐릭터에 혈맹이 존재합니다.", Lineage.CHATTING_MODE_MESSAGE);
							return;
						}
			...
			if(use!=null && !use.isWorldDelete()){
				if(yes){
					if(c.sizeMemberList() < Lineage.clan_max && use.getLevel() > 0){
						...
						ChattingController.toChatting(pc, String.format("혈맹의 최대 가입 인원은 %d명입니다.", Lineage.clan_max), Lineage.CHATTING_MODE_MESSAGE);
						ChattingController.toChatting(use,  String.format("혈맹의 최대 가입 인원은 %d명입니다.", Lineage.clan_max), Lineage.CHATTING_MODE_MESSAGE);
					}
				}else{
	...
	static public void toCallClan(PcInstance pc, boolean yes){
		...
		if(c!=null && c.containsCallList(pc)){
			...
			if(royal != null){
				...
				if (pc.isLock()) {
					ChattingController.toChatting(pc, "현재 상태에서는 이동할 수 없습니다.", Lineage.CHATTING_MODE_MESSAGE);
					return;
				}
		...
		if(c!=null){
			...
			if(pc.getClanGrade() >= 2){
				if (c.getWarClan() != null) {
					ChattingController.toChatting(pc, "전쟁중에 혈맹 문장을 등록할 수 없습니다.", Lineage.CHATTING_MODE_MESSAGE);
					return;
				}
				...
				무인혈맹컨트롤러.update(old_uid, new_uid);
				...
				ChattingController.toChatting(pc, "군주 또는 부군주만 문장 등록이 가능합니다.", Lineage.CHATTING_MODE_MESSAGE);
			}
		}
	}
...
//			World.toSender(S_ClanWar.clone(BasePacketPooling.getPool(S_ClanWar.class), 3, clan.getName(), target.getName()));
	...
	/**
	 * 해당 사용자에 카리 및 퀘스트수행 정보를 참고해서 받아 들일수 있는 최대 혈맹원수 연산후 리턴.
	 */
	static public int getClanMemberMaxSize(String name) {
		//
		Object o = PluginController.init(ClanController.class, "getClanMemberMaxSize", name);
		if (o != null)
			return (Integer) o;
		//
		long objectId = CharactersDatabase.getCharacterObjectId(null, name);
		int level = CharactersDatabase.getCharacterLevel(null, name);
		int cha = CharactersDatabase.getCharacterCharisma(name);
		int size = cha * 2;
		if (level >= 50)
			size = cha * 3;
		// 퀘스트스탭이 100이되야 완료된건데 해당 군주45퀘스트기능이 완벽하게 작업되있는거같지 않음.//성환 수정
		// 그래서 임시로 3보다 같거나 큰경우라면 완료된걸로 봄.
		// : 상위버전은 혈맹집행인을 통해 연합혈맹을 만들 수 있다고 함.

		// 외부에 지정된 혈맹가입제한수 사용할것인가 체크
		// 아닐시 원래 대로 퀘스트 + 카리스마 적용
		if (GameSetting.CLAN_MEMBER_JOIN) {
			size = GameSetting.CLAN_MAX_JOIN_MEMBER;
		} /*
			 * else { Quest q = QuestController.find(objectId, Lineage.QUEST_ROYAL_LV45); if
			 * (q != null && q.getQuestStep() >= 3) { if (level < 50) size = cha * 2; else
			 * size = cha * 3; } }
			 */

		// 신규혈맹 가입인원제한
		PcInstance load = World.findPc(name);
		if (load != null) {
			Clan cc = find(load);
			if (cc.getUid() == 2) {
				size = GameSetting.NEWCLAN_MAX_JOIN_MEMBER;
			}
		}
