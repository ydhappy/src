		...
		try {
			...
			if(AccountDatabase.isAccount(uid, id, pw)) {
				...
				if(item != null) {
					...
					temp.setBress((int)item_bress);
					temp.setQuantity((int)item_quantity);
					temp.setNowTime((int)item_nowtime);
					temp.setDefinite(true);
					// 창고에 등록.
					long inv_id = temp.getItem().isPiles() ? WarehouseDatabase.isPiles(temp.getItem().isPiles(), uid, temp.getItem().getName(), temp.getBress(), 0) : 0;
					if(inv_id > 0)
						WarehouseDatabase.update(temp.getItem().getName(), temp.getBress(), uid, (int)item_count, 0);
					else
						WarehouseDatabase.insert(temp, ServerDatabase.nextItemObjId(), (int)item_count, uid, 0);
					// 메모리 재사용.
