...
// 무인혈맹
public class 무인혈맹컨트롤러 {
	public static List<무인혈맹> list;
	
	static public void init(Connection con) {
		TimeLine.start("무인혈맹컨트롤러..");
		
		list = new ArrayList<무인혈맹>();
		load(con);
	...
	public static void save(Connection con) {
		...
		try {
			...
			for (무인혈맹 ci : getList()) {
				try {
					int i = 1;
					st = con.prepareStatement("INSERT INTO auto_clan_list SET clan_id=?, clan_name=?, gfx=?, 가입레벨=?, 군주=?, 기사=?, 요정=?, 마법사=?, 다크엘프=?, x=?, y=?, map=?, heading=?");
					...
					lineage.share.System.printf("%s : save() -> 혈맹id: %d, 혈맹명: %s\r\n", 무인혈맹컨트롤러.class.toString(), ci.getMasterClanId(), ci.getMasterClanName());
					lineage.share.System.println(e);
				} finally {
	...
	public static void append(무인혈맹 o) {
		synchronized (list) {
			if (!list.contains(o)) {
				list.add(o);
			}
	...
	public static void remove(무인혈맹 o) {
		synchronized (list) {
			if (list.contains(o)) {
				list.remove(o);
			}		
	...
	public static List<무인혈맹> getList() {
		synchronized (list) {
			return new ArrayList<무인혈맹>(list);
		}
	}
	
	public static 무인혈맹 get무인혈맹(int clanId) {
		synchronized (list) {
			for (무인혈맹 temp : list) {
	...
	public static 무인혈맹 find무인혈맹(int objId) {
		synchronized (list) {
			for (무인혈맹 temp : list) {
	...
	public static void insert(PcInstance pc) {
		...
		if (ci == null) {
			...
			String msg = String.format("\\fY               *** [%s] 혈맹에서 혈맹원을 모집합니다. ***", ci.getClanName());
			...
			ChattingController.toChatting(ci, msg, Lineage.CHATTING_MODE_NORMAL);
		} else {
			ci.toTalk(pc, null);
		}
	}
	...
	public static void setGfx(PcInstance pc, 무인혈맹 ci, boolean setGfx) {
	      if (setGfx) {
	...
	public static void insertDB(무인혈맹 ci) {
		Connection con = null;
		PreparedStatement st = null;
		ResultSet rs = null;
	...
	public static void remove(PcInstance pc, int clanId) {
		...
		무인혈맹 ci = get무인혈맹(pc.getGm() > 0 ? clanId : pc.getClanId());
	...
	static public void toJoin(PcInstance pc, 무인혈맹 ci) {
	...
	public static boolean checkZone(PcInstance pc) {
		if (!World.isSafetyZone(pc.getX(), pc.getY(), pc.getMap())) {
			ChattingController.toChatting(pc, "마을에서 가능합니다.", Lineage.CHATTING_MODE_MESSAGE);
			return false;
		}
	...
	public static void move(PcInstance pc, 무인혈맹 ci) {
		if (checkZone(pc)) {
			ci.setHeading(pc.getHeading());
			ci.toTeleport(pc.getX(), pc.getY(), pc.getMap(), false);
		}
