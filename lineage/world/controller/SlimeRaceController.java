...
public class SlimeRaceController {
	...
	public static boolean isSlimeGame = false;
	// 레이서들의 이름목록.
	...
	static public void toTimer(long time){
		if ((Lineage.server_version < 163) || (!isSlimeGame))
			return;
		...
		switch(status){
			case READY:
				if(temp_now_time <= time){
					// 시작전 멘트.
					if(temp_now_idx > 0){
						toMessage( String.format("출발 %d초 전~!", temp_now_idx--) );
			...
			case CLEAR:
				...
				if(temp_now_time <= time){
					// 1분마다 멘트.
					temp_now_time = time + (1000 * 60);
					toMessage( String.format("경기 시작 %d분 전!", temp_now_idx--) );
