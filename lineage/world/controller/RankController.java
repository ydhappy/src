...
public class RankController {
	...
	// 
	static private JSONObject list_json;
	...
	static public void init(Connection con){
		...
		list_json = new JSONObject();
		time_last = System.currentTimeMillis() - Lineage.board_rank_update_delay;
	...
	static public void toTimer(long time){
		if(time_last+Lineage.board_rank_update_delay < time){
			...
			Rank all = getView(0);
			if (all.getList().size() > 0 && rankBronze != null)
				updateBronze(all);
			
			// json 정보 갱신.
			synchronized (list_json) {
				list_json.clear();
				synchronized (list) {
					for(int i=0 ; i<list.size() ; ++i) {
						...
						for(int j=0 ; j<print_limit ; ++j) {
							try {
								list.add( r.getList().get(j) );
							} catch (Exception e) {
								break;
							}
						}
						list_json.put(r.getType(), list);
					}
				}
			}
		}
	...
	public static String toJavaScript() {
		synchronized (list_json) {
			...
			sb.append("var rank=").append( list_json.toJSONString() ).append(";");
			return sb.toString();
		}
	}
	...
	public static int getPvPRank(String name) {
		...
		if (!r.getType().equals("PvP 랭킹")) {
			for(int i=0 ; i<20 ; ++i) {
				try {
					...
					classType = classType.equals("0") ? "군주" : classType.equals("1") ? "기사" : classType.equals("2") ? "요정" : "법사";
			...
			for(int i = 0 ; i < (r.getList().size() < 20 ? r.getList().size() : 20) ; ++i) {
				try {
					...
					classType = classType.equals("0") ? "군주" : classType.equals("1") ? "기사" : classType.equals("2") ? "요정" : "법사";
					sb.append( String.format("%d위 [%s] %s %s킬\r\n", i + 1, classType, name, pkCount) );
				} catch (Exception e) { }
			}
		}
		...
		try {
			...
			if (idx == 5) {
				for (int i = 0; i < r.getList().size(); ++i) {
					if (r.getList().get(i).length() > 0 && r.getList().get(i).startsWith(name + "|")) {
						rank = i + 1;
						break;
					}
				}
				...
				for (int i = 0; i < r.getList().size(); ++i) {
					if (r.getList().get(i).length() > 0
							&& r.getList().get(i).indexOf(String.format("|%s|", name)) > 0) {
						rank = i + 1;
						break;
					}
				}
