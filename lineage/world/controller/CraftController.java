...
public final class CraftController {
	...
	static public boolean isCraft(object o, List<Craft> list, boolean packet){
		...
		for(Craft c : list){
			...
			for(ItemInstance ii : temp_list){
				//
				if (c.getBless() != 0xff && c.getBless() != ii.getBress())
					continue;
				if (c.getEnMin() == 0xff) {
					if (c.getEn() != 0xff && c.getEn() != ii.getEnLevel())
						continue;
				} else {
					if (c.getEnMin() > ii.getEnLevel() || c.getEnMax() < ii.getEnLevel())
						continue;
				}
				//
				if(!ii.isEquipped())
		...
		for (Craft c : list) {
			...
			for (ItemInstance ii : temp_list) {
				...
				if (!ii.isEquipped()) {
					...
					// log
					Log.appendItem(o, "type|제작제거", String.format("item_name|%s", item_name),
							String.format("item_objid|%d", item_objid), String.format("count|%d", temp_count));
				}
			}
	...
	static public int toCraft(PcInstance pc, String item_name, int price, boolean del){
		...
		for(ItemInstance ii : temp_list){
			...
			if(del){
				...
				// log
				Log.appendItem(pc, "type|제작제거", String.format("item_name|%s", ii_name), String.format("item_objid|%d", ii_objid), String.format("count|%d", ii.getCount()));
			}
		}
		return aden;
	}
	...
	static public void toCraft(object o, object target, Item item, long count, boolean packet, int en, int grade, int bless, Object...opt){
		...
		if(opt!=null && opt.length>0)
			target.getInventory().append(temp, count, opt[0], o);
		else
	...
	static public void toCraft2(object o, object target, Item item, long count, boolean packet, int en, int grade, int bless, Object...opt){
		if(o==null || target==null || target.getInventory()==null || item==null || count<=0)
			return;
		...
		ItemDatabase.setPool(temp);
			 ...
			 ChattingController.toChatting(o, String.format("아데나 %d를 획득하였습니다.", count),20);
	}
	 ...
	 * @param i 
	 */
	static public void toCraft(object target, Item item, long count, boolean packet, int en, int i){
		...
		if(packet)
			// %0%o 얻었습니다.
			target.toSender(S_Message.clone(BasePacketPooling.getPool(S_Message.class), 403, count==1 ? item.getName() : String.format("%s (%d)", item.getName(), count)));
	}
	
	static public void toCraft(object target, Item item, long count, boolean packet, int en){
		if(target==null || target.getInventory()==null || item==null || count<=0)
			return;
	...
	static public void toCraftBox(object target, Item item, long count, boolean packet, int en, int bress){
		if(target==null || target.getInventory()==null || item==null || count<=0)
			return;
