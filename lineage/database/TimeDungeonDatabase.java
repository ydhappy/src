...
public final class TimeDungeonDatabase {
	...
	/**
	 * 맵번호로 시간제 던전인지 체크하는 함수 2017-10-07 by all-night
	 */
	...
	/**
	 * 시간제 던전의 이용시간을 모두 사용하였을 경우 처리하는 함수 2017-10-09 by all-night
	 */
		...
		if (pc != null && !pc.isWorldDelete() && !pc.isDead() && !pc.isLock()) {
			...
			case 0:
				...
				ChattingController.toChatting(pc, 
						String.format("기란감옥 이용시간은 %s %d시 초기화 됩니다.", 
	...
	static public void toTimer(long time) {
		...
		// 이용시간 초기화
		if (hour == Lineage.quest_reset_hour && min == 0 && sec == 0)
			resetCheck();
		if (hour == Lineage.quest_reset_hour && min == 0 && sec == 0)
			resetRQues();
		// 기란감옥 이용시간 초기화
	...
	static public void resetGiranDungeonTime() {
		...
		World.toSender(S_ObjectChatting.clone(BasePacketPooling.getPool(S_ObjectChatting.class), String.format("[알림] 기란감옥 이용시간이 초기화 되었습니다")));
	}
	...
	static public void resetCheck() {
		...
		World.toSender(S_ObjectChatting.clone(BasePacketPooling.getPool(S_ObjectChatting.class), String.format("[알림] 출석체크가 초기화 되었습니다")));
	}
	
	static public void resetRQues() {
		
		AccountDatabase.updateRQuest();
		AccountDatabase.updateRQuestKill();
		AccountDatabase.updateRQuestCount();
		AccountDatabase.updateRQuestPlay();
		...
		World.toSender(S_ObjectChatting.clone(BasePacketPooling.getPool(S_ObjectChatting.class), String.format("[알림] 반복 퀘스트 이용횟수가 초기화 되었습니다")));
	}
