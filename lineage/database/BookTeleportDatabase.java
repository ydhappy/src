...
public class BookTeleportDatabase {
	static private List<Book> list;

	/**
	 * 초기화 처리 함수.
	 */
	static public void init(Connection con) {
		TimeLine.start("BookTeleport..");

		list = new ArrayList<Book>();
		PreparedStatement st = null;
		ResultSet rs = null;
		try {
			st = con.prepareStatement("SELECT * FROM book_teleport");
			rs = st.executeQuery();
			...
			lineage.share.System.printf("%s : init(Connection con)\r\n", BookTeleportDatabase.class.toString());
	...
	static public void reload() {
		TimeLine.start("book_teleport 테이블 리로드 - ");
	...
	static public List<Book> getList() {
		return new ArrayList<Book>(list);
	}
	
	static public Book find(int uid) {
		for (Book db : getList()) {
			if (db.getUid() == uid) {
				return db;
			}
	...
	static public void setLoc(Book db, String loc) {
		if (db != null && db.getLoc_list() != null && loc != null) {
			...
			while (st.hasMoreTokens()) {
				try {
					...
					lineage.share.System.printf("%s : setLoc(Book db, String loc) 에러. - [%s]\r\n", BookTeleportDatabase.class.toString(), db.getName());
					lineage.share.System.println(e);
				}
			}
		}
