...
public class LastavardRaid extends Lastavard {
	...
	private void toSpawnBoss(int idx) {
		synchronized (boss_list) {
			...
			MonsterSpawnlistDatabase.toSpawnMonster(boss, World.get_map(boss.getHomeMap()), false, boss.getHomeX(), boss.getHomeY(), boss.getHomeMap(), 0, 0, false, true, false, true);
			synchronized (group_list) {
				//
