...
public class Trade {
	...
	synchronized private boolean isAppend(Character cha, ItemInstance item, long count) {
		try {
			if((int)count < 0 || (int)count > 2000000000)
				return false;
			// 인벤토리에서 제거 가능한지 확인.
			if(cha.getInventory().isRemove(item, count, true, false, false) == false)
				return false;
			// 버프로봇일 경우 무조건 등록.
			if(cha instanceof BuffRobotInstance)
				return true;
			// pc인지 use인지 체크.
			...
			if(isPc) {
				if(use.getInventory().isWeight( Math.round(item.getItem().getWeight()*count) ) == false) {
					ChattingController.toChatting(pc, "상대 소지품이 무거워 더는 등록할 수 없습니다.", 20);
					return false;
				}
			} else {
				if(pc.getInventory().isWeight( Math.round(item.getItem().getWeight()*count) ) == false) {
					ChattingController.toChatting(use, "상대 소지품이 무거워 더는 등록할 수 없습니다.", 20);
					return false;
				}
			}
		} catch (Exception e) {
	...
	synchronized public void toCancel(){
		...
		for( ItemInstance item : pc_list ){
			...
			// log
			Log.appendItem(pc, "type|거래취소", String.format("item_name|%s", item_name), String.format("item_objid|%d", item_objid), String.format("target_name|%s", target_name), String.format("target_objid|%d", target_objid), String.format("drop|%s", Boolean.valueOf(isDrop).toString()));
		}
		for( ItemInstance item : use_list ){
			...
			// log
			Log.appendItem(use, "type|거래취소", String.format("item_name|%s", item_name), String.format("item_objid|%d", item_objid), String.format("target_name|%s", target_name), String.format("target_objid|%d", target_objid), String.format("drop|%s", Boolean.valueOf(isDrop).toString()));
		}
		// 교환창 닫기.
	...
	synchronized public void toOk(){
		...
		for( ItemInstance item : pc_list ){
			...
			// log
			Log.appendItem(use, "type|거래완료", String.format("use_name|%s", use_name), String.format("use_objid|%d", use_objid), String.format("item_name|%s", item_name), String.format("item_objid|%d", item_objid), String.format("target_name|%s", target_name), String.format("target_objid|%d", target_objid), String.format("drop|%s", Boolean.valueOf(isDrop).toString()));
			
			if (!Common.system_config_console) {
				long time = System.currentTimeMillis();
				String timeString = Util.getLocaleString(time, true);
				String log = String.format("[%s]\t [신청]\t [%s->:%s]\t [아이템: %s]", timeString, pc.getName(), use.getName(), itemName);
		...
		for( ItemInstance item : use_list ){
			...
			// log
			Log.appendItem(pc, "type|거래완료", String.format("use_name|%s", use_name), String.format("use_objid|%d", use_objid), String.format("item_name|%s", item_name), String.format("item_objid|%d", item_objid), String.format("target_name|%s", target_name), String.format("target_objid|%d", target_objid), String.format("drop|%s", Boolean.valueOf(isDrop).toString()));
			if (!Common.system_config_console) {
				long time = System.currentTimeMillis();
				String timeString = Util.getLocaleString(time, true);
				String log = String.format("[%s]\t [수락]\t [%s->%s]\t [아이템: %s]", timeString, use.getName(), pc.getName(), itemName);
