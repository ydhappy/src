...
public class Party {
	 ...
	 * @param me
	 */
	public void toSender(BasePacket packet, PcInstance pc, boolean me, boolean only) {
				...
				if (me) {
					if (only) { // 본인에게만 보이는거
						if (pc.getObjectId() == use.getObjectId())
							use.toSender(ServerBasePacket.clone(BasePacketPooling.getPool(ServerBasePacket.class),
									sbp.getBytes()));
					} else { // 전부 보이는거
						use.toSender(ServerBasePacket.clone(BasePacketPooling.getPool(ServerBasePacket.class),
								sbp.getBytes()));
	...
	public void toExp(PcInstance pc, Character target, double exp) {
		...
		if (exp > 0) {
			...
			for (PcInstance use : temp_list) {
				if (!use.isDead()) {
					if (Lineage.party_exp_level_range == 0
							|| Util.isRange(pc.getLevel(), use.getLevel(), Lineage.party_exp_level_range)) {
						// 경험치 지급.
