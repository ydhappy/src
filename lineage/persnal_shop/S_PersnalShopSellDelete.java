...
public class S_PersnalShopSellDelete extends ServerBasePacket {

	static synchronized public BasePacket clone(BasePacket bp, PcInstance pc, List<PersnalShopItem> list) {
		if (bp == null)
			bp = new S_PersnalShopSellDelete(pc, list);
		else
			((S_PersnalShopSellDelete) bp).toClone(pc, list);
		return bp;
	}

	public S_PersnalShopSellDelete(PcInstance pc, List<PersnalShopItem> list) {
		toClone(pc, list);
	}
