...
import lineage.network.util.UByte8;
...
public class PakTools {
    ...
    private static UByte8 ub = new UByte8();
	...
	public static void test() {
		try {
			...
			boolean is380 = false;
			// sprite 파일 읽기.
			BufferedInputStream bis = new BufferedInputStream(new FileInputStream("resource/Text.idx"));
...
//			System.out.println( checksum.getValue() );
			...
			char[] data = uc.fromArray(buffer, buffer.length);
			data = Coder(data, is380 ? 8 : 4, false);
			UByte8 ub = new UByte8();
			buffer = ub.fromArray(data);

			checksum.update(buffer);
			//System.out.println( checksum.getValue() );
			
			int num = true ? 0 : 4;
			int num2 = (buffer.length - num) / (is380 ? 32 : 28);
			//System.out.println( num2 );
			for (int i = 0; i < num2; i++) {
				int index = num + (i * (is380 ? 32 : 28));
				String name = new String(buffer, index+4, 20).trim();
				if(name.length() > 0)
    ...
    public static Pak Decode_Index_FirstRecord(char[] src)
    {
        ...
        return new Pak(ub.fromArray(Decode(destinationArray, 4)), 0);
    }
    ...
    private static char[] Coder(char[] src, int index, boolean IsEncode) {
    	...
    	if (destinationArray.length >= 8) {
            ...
            while (num > 0) {
            	...
            	char[] temp = null;
            	if (IsEncode)
            		temp = sub_403160(buffer2);
                else
            		temp = sub_403220(buffer2);
            	System.arraycopy(temp, 0, destinationArray, destinationIndex, 8);
                destinationIndex += 8;
                num--;
                
                if(destinationIndex > 8*8)
                	break;
            }
    	}
	...
	public static int ToInt32(byte[] data, int idx){
		int result = data[idx++] &0xff;
		result |= data[idx++] << 8 &0xff00;
		result |= data[idx++] << 0x10 &0xff0000;
		result |= data[idx++] << 0x18 &0xff000000;
