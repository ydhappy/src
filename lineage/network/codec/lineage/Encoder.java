	...
	protected Object encode(ChannelHandlerContext ctx, Channel channel, Object msg) throws Exception {
		...
		if(msg instanceof ServerBasePacket){
			...
			if(Socket.PRINTPACKET)
				lineage.share.System.printf("[server] opcode:%d, size:%d, %s\r\n%s\r\n", temp[0]&0xff, temp.length, sbp.toString(), Util.printData(temp, temp.length) );
			//
			if(Lineage.server_version > 360) {
				if( !(sbp instanceof S_Cryptkey) ) {
//					char[] data1 = c.getEncryption380().fromArray(temp);
//					data1 = c.getEncryption380().encrypt_S(data1);
//					temp = c.getEncryption380().fromArray(data1);
					synchronized (c.getEncryption380()) {
						temp = c.getEncryption380().encrypt_S(temp);
					}
				}
				// 데이타 넣기
				...
				if( !(sbp instanceof S_Cryptkey) ) {
					synchronized (c.getEncryption()) {
						temp = c.getEncryption().encrypt(temp);
					}
				}
				// 데이타 넣기
				System.arraycopy(temp, 0, data, 2, length-2);
