	...
	public BasePacket init(LineageClient c) {
		...
		switch(type){
			...
			case 0x05:
				...
				break;
			case 0x07:
				Hp = Lineage.warrior_hp;
				Mp = Lineage.warrior_mp;
				gfx = sex==0 ? Lineage.warrior_male_gfx : Lineage.warrior_female_gfx;
				if(Lineage.warrior_spawn.size()>0)
					fs = Lineage.warrior_spawn.get(Util.random(0, Lineage.warrior_spawn.size()-1));
				break;
			default:
		...
		synchronized (LineageClient.sync_global) {
			...
			try {
				...
				if(!CharactersDatabase.isCharacterName(con, name) && !RobotController.isName(name)){
					if(!CharactersDatabase.isInvalidName(con, name) && name.length()>0){
						if(Str+Dex+Con+Wis+Int+Cha==75){
							if(Hp > 0) {
								...
								// 클레스가 잘못되었을때.
								c.toSender( S_LoginFail.clone(BasePacketPooling.getPool(S_LoginFail.class), S_LoginFail.REASON_WRONG_CLASS) );
							}
						}else{
							// 스탯합이 잘못 됬을때.
