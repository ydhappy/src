			...
			try {
				...
				if(CharactersDatabase.isCharacter(c.getAccountUid(), name)){
					if(!CharactersDatabase.isInvalidName(con, name)){
						...
						long obj_id = CharactersDatabase.getCharacterObjectId(con, name);
						int clan_id = CharactersDatabase.getCharacterClanId(con, name);
						// 로그 기록.
						if(Log.isLog(null))
						...
						try {
							...
							st.close();
							//
							CharactersDatabase.remove(obj_id, name);
							QuestController.remove(obj_id);
//							if(clan_id > 0)
//								ClanController.remove(clan_id, name);
						} catch (Exception e) {
