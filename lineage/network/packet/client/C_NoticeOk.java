...
import lineage.network.packet.server.S_TamCount;
		...
		if(c.getAccountUid()>0){
			...
			try {
				...
				if(Lineage.server_version>=380)
					c.toSender( S_BaseStat.clone(BasePacketPooling.getPool(S_BaseStat.class), 0x16) );
				// 소유중인 케릭갯수 전송
				...
				// ?
				if(Lineage.server_version>=380)
					c.toSender( S_BaseStat.clone(BasePacketPooling.getPool(S_BaseStat.class), 0x0a) );
				// 소유중인 케릭터정보 전송
				if(length>0){
					PreparedStatement st = null;
					ResultSet rs = null;
					try {
						st = con.prepareStatement("SELECT name, clanNAME, class, sex, lawful, nowHP, nowMP, ac, level, str, dex, con, wis, cha, inter, register_date FROM characters WHERE account_uid=?");
						...
						while(rs.next()){
							...
							try { c.toSender( S_Character.clone(BasePacketPooling.getPool(S_Character.class), Opcodes.S_OPCODE_CHARLIST, name, clan, type, sex, lawful, hp, mp, ac, level, _str, _dex, _con, _wis, _cha, _int, Util.getLocaleString(register, false)) ); } catch (Exception e) {
								c.toSender( S_Character.clone(BasePacketPooling.getPool(S_Character.class), Opcodes.S_OPCODE_CHARLIST, name, clan, type, sex, lawful, hp, mp, ac, level, _str, _dex, _con, _wis, _cha, _int, null) );
							}
						}
					} catch (Exception e) {
				...
				// ?
				if(Lineage.server_version>=380) {
					c.toSender( S_BaseStat.clone(BasePacketPooling.getPool(S_BaseStat.class), 0x40) );
					c.toSender( S_TamCount.clone(BasePacketPooling.getPool(S_TamCount.class), c.getAccountTam()) );
				}
