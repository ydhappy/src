...
public class S_UserShopList extends S_Inventory {
	...
	static synchronized public BasePacket clone(BasePacket bp, PcInstance pc, PcInstance use, Useshop us, boolean buy){
		if(bp == null)
			bp = new S_UserShopList(pc, use, us, buy);
		else
			((S_UserShopList)bp).clone(pc, use, us, buy);
		return bp;
	}
	
	public S_UserShopList(PcInstance pc, PcInstance use, Useshop us, boolean buy){
		clone(pc, use, us, buy);
	}
	
	public void clone(PcInstance pc, PcInstance use, Useshop us, boolean buy){
		clear();
		...
		if(buy){
			...
			for(int i=0 ; i<us.getSell().size() ; ++i){
				...
				if(ii.getUsershopSellCount() > 0){
					// 아이템 찾아보기.
					ItemInstance use_ii = use.getInventory().find(ii.getItem().getName(), ii.getEnLevel(), ii.getBress());
					// 존재할경우만.
					if(use_ii!=null && !use_ii.isEquipped() && !list.contains(use_ii)){
