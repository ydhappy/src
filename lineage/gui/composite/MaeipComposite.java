...
public class MaeipComposite extends Composite {
	
	private StyledText chatting;
	private org.eclipse.swt.graphics.Color color_drop = SWTResourceManager.getColor(243, 247, 0);
	private org.eclipse.swt.graphics.Color color_pickup = SWTResourceManager.getColor(255, 0, 0);
	 ...
	 * @param style
	 */
	public MaeipComposite(Composite parent, int style) {
		super(parent, style);
	...
	public void toLog(String msg){
		...
		// 색상 입히기.
		if (msg.contains("매입"))
			chatting.setStyleRange( getHighlightStyle(log_now_pos, msg.length(), color_drop, SWT.BOLD) );
		else
			chatting.setStyleRange( getHighlightStyle(log_now_pos, msg.length(), color_pickup, SWT.BOLD) );
