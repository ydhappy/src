...
public class ChattingComposite extends Composite {
	...
	public ChattingComposite(Composite parent, int style) {
		...
		GridData gd_scroll = new GridData(SWT.LEFT, SWT.CENTER, false, false,
				2, 1);
		...
		global_lock.setLayoutData(new GridData(SWT.LEFT, SWT.CENTER, true,
				false, 1, 1));
		...
		global.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, false,
				1, 1));
		global.setText("전체");
		...
		chatting.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, true, 10,
				1));
		...
		text.setBackground(SWTResourceManager.getColor(24, 24, 24));
		...
		text.addKeyListener(new KeyAdapter() {
				...
				case 13: // 엔터
					...
					if (Main.running) {
						...
						if (msg.startsWith("\"")) {
							...
							ChattingController.toChatting(null, text.getText(),
									3);
						}
					}
					text.setText("");
			...
			public void keyPressed(KeyEvent e) {
				switch (e.keyCode) {
				case 39: // '
					bool_chatting_whisper = e.stateMask == SWT.SHIFT
		...
		GridData gd_button = new GridData(SWT.FILL, SWT.CENTER, false, false,
				1, 1);
	...
	public void toNormal(object o, String msg) {
		if (normal.getSelection()) {
			msg = String.format("\r\n%s %s : %s",
					Util.getLocaleString(System.currentTimeMillis()),
					o.getName(), msg);
			chatting.append(msg);
			if (!scroll.getSelection())
				chatting.setTopIndex(chatting.getVerticalBar().getMaximum());
			// 색상 입히기.
			chatting.setStyleRange(getHighlightStyle(chatting_now_pos,
					msg.length(), color_normal, SWT.BOLD));
	...
	public void toTrade(object o, String msg) {
		if (trade.getSelection()) {
			msg = String.format("\r\n%s [%s] %s", Util.getLocaleString(System
					.currentTimeMillis()), o == null ? "******" : o.getName(),
					msg);
			chatting.append(msg);
			if (!scroll.getSelection())
				chatting.setTopIndex(chatting.getVerticalBar().getMaximum());
			// 색상 입히기.
			chatting.setStyleRange(getHighlightStyle(chatting_now_pos,
					msg.length(), color_trade, SWT.BOLD));
			chatting_now_pos += msg.length();
		}
	}
	...
	public void toClan(object o, String msg) {
		if (clan.getSelection()) {
			msg = String.format("\r\n%s {%s}{%s} %s", Util.getLocaleString(System.currentTimeMillis()), o.getClanName(), o.getName(), msg);
			chatting.append(msg);
			if (!scroll.getSelection())
				chatting.setTopIndex(chatting.getVerticalBar().getMaximum());
			// 색상 입히기.
	 ...
	 * @param msg
	 */
	public void toClanSafe(object o, String msg) {
		if (clan.getSelection()) {
			msg = String.format("\r\n%s {%s}{{%s}} %s", Util.getLocaleString(System.currentTimeMillis()), o.getClanName(), o.getName(), msg);
			chatting.append(msg);
			if (!scroll.getSelection())
				chatting.setTopIndex(chatting.getVerticalBar().getMaximum());
			// 색상 입히기.
			chatting.setStyleRange(getHighlightStyle(chatting_now_pos, msg.length(), color_clan_safe, SWT.BOLD));
			chatting_now_pos += msg.length();
		}
	}
	...
	public void toParty(object o, String msg) {
		if (party.getSelection()) {
			...
			// 색상 입히기.
			chatting.setStyleRange(getHighlightStyle(chatting_now_pos,
					msg.length(), color_party, SWT.BOLD));
			chatting_now_pos += msg.length();
		}
	}
	...
	public void toWhisper(object o, String name, String msg) {
		if (whisper.getSelection()) {
			msg = String.format("\r\n%s (%s)->(%s) %s", Util
					.getLocaleString(System.currentTimeMillis()),
					o == null ? "******" : o.getName(), name, msg);
			chatting.append(msg);
			if (!scroll.getSelection())
				chatting.setTopIndex(chatting.getVerticalBar().getMaximum());
			// 색상 입히기.
			chatting.setStyleRange(getHighlightStyle(chatting_now_pos,
					msg.length(), color_whisper, SWT.BOLD));
			chatting_now_pos += msg.length();
		}
	}
	...
	public void toMessage(String msg) {
		msg = String.format("\r\n%s %s",
				Util.getLocaleString(System.currentTimeMillis()), msg);
		chatting.append(msg);
		if (!scroll.getSelection())
			chatting.setTopIndex(chatting.getVerticalBar().getMaximum());
		// 색상 입히기.
		chatting.setStyleRange(getHighlightStyle(chatting_now_pos,
				msg.length(), color_message, SWT.BOLD));
		chatting_now_pos += msg.length();
	}

	private StyleRange getHighlightStyle(int startOffset, int length,
			org.eclipse.swt.graphics.Color color, int font_style) {
